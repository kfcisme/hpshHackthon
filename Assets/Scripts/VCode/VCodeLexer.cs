    ...
    public sealed class VCodeLexer
    {
        ...
        public List<Token> Lex(string source)
        {
            ...
            while (i < source.Length) {
                ...
                if (char.IsLetter(c) || c=='_') { int begin=i; while (i<source.Length && (char.IsLetterOrDigit(source[i]) || source[i]=='_')) { i++; column++; } string word=source.Substring(begin,i-begin); result.Add(new Token(Keywords.TryGetValue(word,out var keywordType)?keywordType:TokenType.Identifier,word,line,start)); continue; }
