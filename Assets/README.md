...
The initial usable loop is: assign `CanvasRenderer`, `IDEEditorController`, and `LevelScenePresenter` in `Level`, wire its target `RawImage`, call `Compile` from a UI Button, and give `LevelDefinition` a 64×64 target texture. V-Code, draw-command execution, pixel comparison, anomaly classes, and local JSON player progress are implemented in source.
